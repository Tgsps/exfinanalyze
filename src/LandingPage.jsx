...
function WaitlistForm({onSuccess}){
  ...
  const valid=form.name.trim()&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)&&form.role&&form.size;
  ...
  const submit=async()=>{
    if(!valid){setErr("Please fill in all required fields.");return;}
...
function AdminPanel(){
  ...
  useEffect(()=>{sb.getAll().then(l=>{setList(Array.isArray(l)?l:[]);setLoading(false)});},[]);
      ...
      <section id="pricing" style={{padding:"96px 40px",background:"var(--white)"}}>
        <div className="wrap">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:22,maxWidth:900,margin:"0 auto"}}>
            {[
              {tier:"Starter",price:"Free",period:"forever",desc:"For individual accountants exploring AI-assisted analysis.",features:["5 documents/month","Basic extraction","ASC 842 lease classifier","Email support"],cta:"Get started free",hi:false},
              {tier:"Professional",price:"$49",period:"/month",desc:"For growing teams who need speed and accuracy at month-end.",features:["Unlimited documents","AI Shadow Reviewer","Month-end close dashboard","Priority support","Export to QuickBooks & Xero"],cta:"Join waitlist — 40% off",hi:true},
              {tier:"Team",price:"Custom",period:"",desc:"For accounting firms and corporate finance departments.",features:["Everything in Professional","Multi-entity close management","SSO & audit trail","Dedicated onboarding","SLA + SOC 2 Type II"],cta:"Contact sales",hi:false},
            ].map((p,i)=>(
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:28,borderBottom:"1px solid #2A2820",marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:24,fontSize:13,color:"#6B6963"}}>
              {["Privacy","Terms","Security","Contact"].map(l=>(
                <a key={l} href="#" style={{textDecoration:"none",color:"inherit",transition:"color .2s"}} onMouseEnter={e=>e.target.style.color="#E8E6DF"} onMouseLeave={e=>e.target.style.color="#6B6963"}>{l}</a>
              ))}
            </div>
          </div>
